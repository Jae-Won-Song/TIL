...
--> 서포트 벡터가 위치한 `Plus & Minus plane에 약간의 여유변수(Slack Variable)을 두는 것`으로 해결
...
--> Original data가 놓여있는 차원을 `비선헝 매핑을 통해 고차원 공간으로 변환`하는 것으로 해결
...
with open('raw_pos_tagged.pkl', 'wb') as f:
  pickle.dump(raw_pos_tagged, f) # dump => 저장함수
```

- pickle 파일로부터 파이썬 변수를 불러오는 방법 
```
with open('raw_pos_tagged.pkl','rb') as f:
    data = pickle.load(f) # load=> 불러오는 함수
```
